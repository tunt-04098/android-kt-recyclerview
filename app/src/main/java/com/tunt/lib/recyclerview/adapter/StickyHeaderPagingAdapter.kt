 ...
 */
abstract class StickyHeaderPagingAdapter<Item>(hasHeader: Boolean = false,
                                               hasFooter: Boolean = false,
                                               allowDeselectItem: Boolean = false) : PagingRecyclerAdapter<Item>(hasHeader, hasFooter, allowDeselectItem), StickyHeaderAwareAdapter<Item> {
    ...
    override fun getHeaderId(position: Int): Long = when {
        (hasHeader && position == 0) || (hasFooter && position == itemCount - 1)
        || (!isIdle() && position == getCollectionItemCount() + (if (hasHeader) 1 else 0)) -> -1
        else -> {
